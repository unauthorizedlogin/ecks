...
Next alphabetically would probably be your next addon folder after `item_creator` (depending on your addon list). Send the next creator tool and I'll append the next section.

This is enough. This changes the description slightly because the XP Tool Dock is not just a simulator — it is a **progression authoring and balancing suite**.

I would still avoid documenting every panel. The important identity pieces are:

* Shared editor state (`XPToolContext`)
* Swappable XP curves
* Swappable formula profiles
* Curve/growth generation
